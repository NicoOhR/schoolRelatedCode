...
gradient boosting trees. In the first section, the theory of base decision
...
trees and ensemble methods are tested against multiple data sets,
...
A decision trees is a non-parametric, supervised model which can be
...
decision trees are a very popular tool. Decision tree's are also
...
called ensemble learning methods. Each one addresses a particular
...
\begin{itemize}
  \item Random forest methods
    ...
    subset of features.
  \item Boosting methods train trees additively and sequentially. By
...
\begin{algorithm}
  ...
  \caption{Decision Tree}\label{alg:}
  \begin{algorithmic}[1]
...
$d_{\text{max}}$ a decision tree will always
...
in the gradient boosting model, we treat the model $F$ itself as the
...
As discussed in the decision tree section, a single deep decision
...
computationally efficient manner, using domain specific loss
...
decision tree, gradient boosting model, and XGBoost model is a
...
scikit learn. For instance, the custom GBM and sklearn GBM almost
...
custom and standard decision trees exhibit higher variance when the
data is noiser while the ensemble methods are slightly more unstable
...
extremely. The L1 and L2 regularizations
