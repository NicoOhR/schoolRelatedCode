...
\begin{proof}
  ...
  Notice that, because $f$ is increasing,
  \[
    U(f,P) = \sum f(x_{i+1})(x_{i+1} - x_i) \quad L(f,P) = \sum
    f(x_i)(x_{i+1} - x_i)
  \]
  For any partition $P$. Thus
  \[
    U(f,P) - L(f,P) = \sum (f(x_{i+1}) - f(x_i))(x_{i+1} - x_i)
  \]
  Set the partition to be equally spaced, such that $\forall i, c:=
  x_{i+1} - x_i$ for some constant $c$. So the above sume becomes
  \[
    U(f,P) - L(f,P) = \sum (f(x_{i+1}) - f(x_i))c
  \]
  Clearly, successive terms cancel, so
  \[
    U(f,P) - L(f,P) = (f(b) - f(a))c
  \]
  So for $c < \frac{\varepsilon}{f(b) - f(a)}$, which we can always
  set, we get that:
  $$
  U(f, P) - L(f,P) =(f(b) - f(a))c < (f(b) -
  f(a))\frac{\varepsilon}{f(b) - f(a)} = \varepsilon
  $$
...
\begin{proof}
  \begin{enumerate}
    ...
    \item
      \begin{align*}
        |f_k(x) - f(x)| &= \left | \sum_{n=k}^\infty f_n \right | \\
        &= \left | \sum_{n=k}^\infty \frac{nx}{1+nx^2} \right | \\
      \end{align*}
      So, if $\forall \varepsilon > 0, \exists k$ such that
      \[
        \left | \sum_{n=k}^\infty \frac{nx}{1+nx^2} \right | <
        \varepsilon \implies |f_k(x) - f(x)| < \varepsilon
      \]
      This is of course just the cauchy criterion for convergence, so
      it is sufficient to show that $\forall x \in (1, \infty0)$
      \[
        \sum \frac{nx}{1+nx^2}
      \]
      Is convergent. By the ratio test:
      \begin{align*}
        \frac{\frac{(n+1)x}{1+(n+1)x^2}}{\frac{nx}{1+nx^2}} &=
        \frac{((n+1)x)(1+nx^2)}{nx(1+(n+1)x^2)} \\
        &= \frac{n+1}{n}\frac{1+nx^2}{1+nx^2+x^2}
      \end{align*}
      As $n \to \infty$ then
      \[
        \frac{n+1}{n}\frac{1+nx^2}{1+nx^2+x^2} \to \frac{1}{x^2}
      \]
      Which, for $x > 1$, is smaller than $1$, thus the function
      uniformly converges for $x \in (1, \infty)$
    \item Following from the last part, for $x < 1 \implies
      \frac{1}{x^2} > 1$, so by the ratio test, the function diverges.
  \end{enumerate}
\end{proof}
\end{document}
