...
\begin{proof}
  ...
  \begin{align*}
    \left|\frac{2nx}{(1+nx^2)^2}\right| < \left|\frac{2nx}{n^2x^4}\right| =
    \left|\frac{2}{x^3}\frac{1}n\right|
  \end{align*}
  Clearly, the above bounding sequence covnerges to $0$ as $n$ goes
  to infinity, therefore the sequence converges pointwise to $0$. To
  show that the sequence does not converge to $0$ uniformly, we want
  to find an $x$ such that
  \[
    \left|\frac{2nx}{(1+nx^2)^2}\right| \geq \varepsilon
  \]
  For all $n$. Consider $x = \frac{1}n$
  \[
    \left|\frac{2}{(1+\frac{1}{n})^2}\right| \geq \varepsilon
  \]
  Notice that $1 + \frac1{n} \leq 2, \forall n \in \mathbb{N}$
  $$
  \left|f'(\frac1n)\right| = \left|\frac{2}{(1+\frac{1}n)^2}\right|
  \geq \frac{2}{2^2}
  $$
  So for any $\varepsilon < \frac{1}2$ we get that
  \[
    \left|f'(\frac1n)\right| > \varepsilon
  \]
\end{proof}
\end{document}
