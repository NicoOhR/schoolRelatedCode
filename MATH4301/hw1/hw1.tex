...
\item
  ...
  \item let $m_b = \inf B$ and  $s_a =  \sup A$
    \begin{enumerate}
      ...
      \item From part (a), $s_a - m_b$ is an upper bound for $A - B$. To prove that $u \geq s_a - m_b$ : 
        \begin{itemize}
          \item Construct a set $S = A - m_b = \{a - m_b \mid \forall a \in A \}$ for which $\sup S = s_a - m_b$ since $s_a \geq a \forall a \in A$ 
          \item and using the result from part (b), $a - m_b \leq u \implies \sup(S) = s_a  - m_b \leq u$.
        \end{itemize}
          Demonstrating that $s_a - m_b$ is the least upper bound, or that $\sup(A - B) = \sup A - \inf B$
    \end{enumerate}
\end{enumerate}
