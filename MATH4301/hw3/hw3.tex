...
False 
\begin{proof}
  We are given that the sequence (call it $x_n$) is increasing and divergent, then by the monotone convergance theorem, we know it must not be bounded above, that is, there does not exist a real number $M$ such that for all $n \in \mathbb{N}$ $x_n < M$ and $x_{n+1} > x_n$.  Lets assume for the sake of contradiction that we can construct an increasing integer sequence $n_i$ such that $x_{n_i} \to L$. 
